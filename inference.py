...
def detect3d(
    ...
    save_result,
    output_path,
    json_file_path = '../scene9_frames.json'
    ):
    ...
    for i, img_path in enumerate(imgs_path):
        ...
        # print(f'dets: {dets} , {dets[0].detected_class}')
        ...
        for en,det in enumerate(dets):
            ...
            # print(f'box_2d: {box_2d} , detected_class: {detected_class}')
            ...
            # print(f'orient: {orient} conf: {conf} dim: {dim}')
    ...
    with open(json_file_path, 'w') as f:
            json.dump(Frame_count, f, indent=4)
...
def plot3d(
    ...
    # print(f'location: {location} orient: {orient} dimensions: {dimensions}')

    corners, R, center, box_3d = plot_3d_box(img, proj_matrix, orient, dimensions, location) # 3d boxes

    return location , corners, R, center, box_3d
...
def parse_opt():
    ...
    parser.add_argument('--classes', default=[0, 2, 3, 5, 9], nargs='+', type=int, help='filter by class: --classes 0, or --classes 0 2 3')
    ...
    parser.add_argument('--json_path', type=str, default='frame.json', help='Save output pat')
...
def main(opt):
    ...
    detect3d(
        ...
        source = f"{parent_directory}/scene4/front_frames/",
        calib_file=opt.calib_file,
        show_result=opt.show_result,
        save_result=opt.save_result,
        output_path=opt.output_path,
        json_file_path = opt.json_path #'../scene9_frames.json'
    )
...
# python inference.py --source eval/UgM4/ --reg_weights weights/resnet18.pkl --model_select resnet18 --output_path runs/ --save_result --show_result